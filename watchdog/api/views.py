...
from watchdog.db import get_db
...
def _request_graphite(items, start_str, end_str):
    try:
        ...
        if resp.status_code != 200:
            ...
            raise Exception('Request Graphite Error')

    except Exception as ex:
        app.logger.info(ex)
        raise Exception('Graphite Service Error')

    app.logger.debug("data: %s" % resp.json())
    return resp.json()
...
def item_list():
    ...
    """
    db = get_db()
    data = db.execute('select * from monitors').fetchall()
